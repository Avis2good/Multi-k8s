docker built -t avis2good/multi-client:latest -t avis2good/multi-client:$SHA -f ./client/Dockerfile ./client
docker built -t avis2good/multi-server:latest -t avis2good/multi-server:$SHA -f ./server/Dockerfile ./server
docker built -t avis2good/multi-worker:latest -t avis2good/multi-worker:$SHA -f ./worker/Dockerfile ./worker
